...
describe('thermal receipt safety', () => {
  ...
  it('does not include customer markup as executable html', () => {
    const html = receiptHtml({ invoice:'INV-1', customer:{...demoCustomers[0],name:'<img src=x onerror=alert(1)>'}, lines:[{product:demoProducts[0],unit:demoProducts[0].units[0],qty:1,discount:0}],paid:5000,total:3500 });
