...
function PaymentDialog({ customer,cart,total,onClose,onDone }:{customer:Customer;cart:CartLine[];total:number;onClose:()=>void;onDone:(invoice:string,printFailed:boolean)=>void}) {
  ...
  function pickMethod(m:PaymentMethod){setMethod(m);setError('');setIsDebt(false);if(m.type==='cash')setPaid(total);else{setPaid(total);setReference('');}}
  async function submit(){
    ...
    if(isCash&&isDebt&&shortfall>0&&isGeneralCustomer)return setError('Pilih pelanggan terdaftar untuk transaksi piutang, bukan Pelanggan Umum.');
    if(saving)return;
  ...
  const submitDisabled=saving||methodsLoading||!method||(isCash&&!isDebt&&paid<total)||(isCash&&isDebt&&shortfall>0&&isGeneralCustomer);
  return <Modal title="Pembayaran" onClose={onClose}><div className="payment-total"><span>Total pembayaran</span><strong>{money.format(total)}</strong></div>
    ...
    {method&&isCash&&<><label>Uang diterima<input className="money-input" type="number" min={0} value={paid} onChange={e=>setPaid(Number(e.target.value))} onFocus={e=>e.currentTarget.select()} /></label>
      ...
      {shortfall<=0?<div className="change"><span>Kembalian</span><strong>{money.format(Math.max(0,paid-total))}</strong></div>:<label className="checkbox-row"><input type="checkbox" checked={isDebt} onChange={e=>setIsDebt(e.target.checked)} /> Pelanggan berhutang (piutang)</label>}
