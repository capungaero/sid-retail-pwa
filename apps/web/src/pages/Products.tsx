...
export function Products() {
  ...
  // Tracks the currently-displayed blob: URL (from createObjectURL) so it can be revoked
  // before the next one replaces it — product.photoUrl from the server is a real URL, never
  // stored here, so we never revoke something we didn't create.
  const photoObjectUrlRef = useRef('');
  function releasePhotoObjectUrl() {
    if (photoObjectUrlRef.current) { URL.revokeObjectURL(photoObjectUrlRef.current); photoObjectUrlRef.current = ''; }
  }
  ...
  useEffect(load, []);
  function openEditor(product: Product) { releasePhotoObjectUrl(); setPhotoFile(null); setPhotoPreview(product.photoUrl ?? ''); setEditing(product); }
  function closeEditor() { releasePhotoObjectUrl(); setEditing(null); }
  async function handlePhotoBlob(file: Blob) {
    try { const compressed = await compressImage(file); releasePhotoObjectUrl(); const url = URL.createObjectURL(compressed); photoObjectUrlRef.current = url; setPhotoFile(compressed); setPhotoPreview(url); }
  ...
  async function submit(e: FormEvent<HTMLFormElement>) { e.preventDefault(); if (!editing) return; const form = new FormData(e.currentTarget); // No client UUID for a brand-new product: leaving id empty makes saveProduct() in api.ts
    // correctly POST /products (ProductController::store) instead of PUT /products/{uuid},
    // which used to work only by coincidence (see log.txt / audit notes on this fix).
    const next: Product = { ...editing, id: editing.id, code: String(form.get('code')), barcode: String(form.get('barcode')), name: String(form.get('name')), category: String(form.get('category')), minStock: Number(form.get('minStock')), cost: Number(form.get('cost')), units: [{ ...editing.units[0], name: String(form.get('unit')), price: Number(form.get('price')) }] }; if (!next.code || !next.name || next.units[0].price < 0) return setError('Kode, nama, dan harga valid wajib diisi.'); try { let saved = await saveProduct(next); if (photoFile) { try { const { photoUrl } = await uploadProductPhoto(saved.id, photoFile); saved = { ...saved, photoUrl }; } catch (photoErr) { setError(photoErr instanceof Error ? photoErr.message : 'Barang tersimpan, tetapi unggah foto gagal.'); } } setProducts(current => current.some(p => p.id === saved.id) ? current.map(p => p.id === saved.id ? saved : p) : [...current, saved]); closeEditor(); } catch (err) { setError(err instanceof Error ? err.message : 'Gagal menyimpan barang'); } }
