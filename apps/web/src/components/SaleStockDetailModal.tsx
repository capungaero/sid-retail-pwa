...
export function SaleStockDetailModal({ sale, onClose }: { sale: SaleRecord; onClose: () => void }) {
  ...
  return <div className="modal-overlay" role="presentation"><section ref={ref} className="modal" role="dialog" aria-modal="true" aria-labelledby="sale-stock-title">
    ...
    <div className="table-wrap"><table><thead><tr><th>Barang</th><th className="numeric">Stok awal</th><th className="numeric">Terjual</th><th className="numeric">Harga</th><th className="numeric">Sisa stok</th></tr></thead><tbody>
      {sale.lines.flatMap((l, i) => { const ex = exchangeFor(l.productId, l.unit); const rows = [
      ...
      ]; if (ex) rows.push(<tr key={`${i}-ex`} className="muted"><td><ArrowLeftRight size={14} style={{ verticalAlign: 'middle', marginRight: 4 }} />{ex.newProductName}<br /><small className="muted">{ex.newUnit}</small></td><td className="numeric mono">—</td><td className="numeric mono">{number.format(ex.newQty)}</td><td className="numeric mono">{money.format(ex.newLineValue)}</td><td className="numeric mono">—</td></tr>); return rows; })}
    </tbody></table></div>
